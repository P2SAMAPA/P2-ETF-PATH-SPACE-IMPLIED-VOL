...
st.set_page_config(page_title="Path-Space Implied Volatility", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Path-Space Implied Volatility</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Signature-based implied vol | Path-dependent functional | Beyond strike-based IV</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Path-Space IV")
...
st.sidebar.markdown(f"**Signature depth:** {config.SIG_DEPTH} | **Horizon:** {config.HORIZON}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'psiv_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Path-space IV: {etf['psiv_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized IV"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized IV", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Path-Space Implied Volatility (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Path-space implied volatility** models IV as a functional on path space.
        - Uses **signature methods** (iterated integrals) to capture path-dependent features.
        - Goes beyond strike-based IV to include path history.
        - Higher path-space IV → expected volatility is higher → more expensive options.
        - Macro variables modulate the path-space IV.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the path-space IV scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Path-Space Implied Volatility | Signature-based IV functional")
